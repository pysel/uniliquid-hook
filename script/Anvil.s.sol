...
contract UniliquidHookScript is Script, DeployPermit2 {
    ...
    function run() public {
        ...
        // ----------------------------- //
        vm.broadcast();
        UniliquidHook uniliquidHook = new UniliquidHook{salt: salt}(manager);
        require(address(uniliquidHook) == hookAddress, "UniliquidHookScript: hook address mismatch");
        ...
        vm.startBroadcast();
        testLifecycle(address(uniliquidHook));
        vm.stopBroadcast();
    }
